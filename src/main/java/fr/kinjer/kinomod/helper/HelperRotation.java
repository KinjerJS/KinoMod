...
public final class HelperRotation {

	public static final HelperRotation IDENTITY = new HelperRotation(0, 0, 0, 1);
	...
	public HelperRotation(double d, double d1, double d2, double d3) {
		x = d1;
		y = d2;
		z = d3;
		s = d;
	...
	public static HelperRotation aroundAxis(double ax, double ay, double az, double angle) {
		angle *= 0.5D;
		double d4 = Math.sin(angle);
		return new HelperRotation(Math.cos(angle), ax * d4, ay * d4, az * d4);
	}

	public HelperRotation multiply(HelperRotation quat) {
		...
		return new HelperRotation(d1, d2, d3, d);
	}

	public HelperRotation rightMultiply(HelperRotation quat) {
		double d = s * quat.s - x * quat.x - y * quat.y - z * quat.z;
		double d1 = s * quat.x + x * quat.s + y * quat.z - z * quat.y;
		double d2 = s * quat.y - x * quat.z + y * quat.s + z * quat.x;
		double d3 = s * quat.z + x * quat.y - y * quat.x + z * quat.s;
	...
	public HelperRotation normalize() {
		double d = mag();
		if (d == 0.0D) {
			return this;
		} else {
			d = 1.0D / d;
			return new HelperRotation(x * d, y * d, z * d, s * d);
		}
	}

	public HelperVector rotate(HelperVector vec) {
		...
		return new HelperVector(vx, vy, vz);
	}
	...
	public boolean equals(Object o) {
		return o instanceof HelperRotation
				&& ((HelperRotation) o).x == x
				&& ((HelperRotation) o).y == y
				&& ((HelperRotation) o).z == z
				&& ((HelperRotation) o).s == s;
	}
