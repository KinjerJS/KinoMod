...
public class BaseKinoBaubleBelt extends Item implements IBauble {
	...
	public BaseKinoBaubleBelt(String name) {
		super();
		...
		if(!evt.getEntityLiving().world.isRemote
			...
			for(int i = 0; i < inv.getSlots(); i++) {
				...
				if (!stack.isEmpty() && stack.getItem().getRegistryName().getResourcePath().equals(KinoMod.MODID)) {
					((BaseKinoBaubleBelt) stack.getItem()).onUnequipped(stack, evt.getEntityLiving());
				}
			}
		}
	}
