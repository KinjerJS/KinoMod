...
import fr.kinjer.kinomod.client.gui.GuiCrystaliser;
import fr.kinjer.kinomod.containers.ContainerCrystaliser;
...
public class TileCrystaliser extends TileMachineBase {
...
//		SIDE_CONFIGS[TYPE].sideTypes = new int[] { NONE, INPUT_ALL, OUTPUT_PRIMARY, OUTPUT_SECONDARY, OUTPUT_ALL, OPEN,
//				OMNI };
...
//		GameRegistry.registerTileEntity(TileTransposer.class, "thermalexpansion:machine_transposer");
		GameRegistry.registerTileEntity(TileCrystaliser.class, "kinomod:tile_crystaliser");
		config();
	}
...
//		basePower = ThermalExpansion.CONFIG.getConfiguration().getInt("BasePower", category, basePower, MIN_BASE_POWER,
//				MAX_BASE_POWER, comment);
//
...
//		ENERGY_CONFIGS[TYPE].setDefaultParams(basePower, smallStorage);
	...
	private boolean canStartHandler() {
		...
		IFluidHandlerItem handler = inventory[1].getCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY,
				null);
		...
		if (!extractMode) {
			return tank.getFluid() != null && tank.getFluidAmount() > 0
					&& handler.fill(
							new FluidStack(tank.getFluid(), Math.min(tank.getFluidAmount(), Fluid.BUCKET_VOLUME)),
							false) > 0;
		} else {
	...
	private void processStartHandler() {
		...
		if (!extractMode) {
			...
			renderFluid = tank.getFluid() == null ? handlerStack == null ? null : handlerStack.copy()
					: tank.getFluid().copy();
		}
		if (renderFluid == null) {
	...
	private boolean fillHandler() {
		...
		IFluidHandlerItem handler = inventory[1].getCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY,
				null);
		int filled = tank.getFluid() == null ? 0
				: handler.fill(new FluidStack(tank.getFluid(), Math.min(tank.getFluidAmount(), Fluid.BUCKET_VOLUME)),
						true);
		...
		if (filled > 0) {
			...
			return tankProperties[0].getContents() != null
					&& tankProperties[0].getContents().amount >= tankProperties[0].getCapacity();
		}
		return true;
	}
	...
	protected boolean canStart() {
		...
		if (!extractMode) {
			...
			return ItemHelper.itemsIdentical(inventory[2], output)
					&& inventory[2].getCount() + output.getCount() <= output.getMaxStackSize();
		} else {
			...
			ItemStack output = recipe.getOutput();
			return output.isEmpty() || ItemHelper.itemsIdentical(inventory[2], output)
					&& inventory[2].getCount() + output.getCount() <= output.getMaxStackSize();
		}
	}
	...
	protected boolean hasValidInput() {
		...
		TransposerRecipe recipe = extractMode ? TransposerManager.getExtractRecipe(inventory[1])
				: TransposerManager.getFillRecipe(inventory[1], tank.getFluid());
	...
	private void transferOutputFluid() {
		...
		int side;
		FluidStack outputBuffer = new FluidStack(tank.getFluid(),
				Math.min(tank.getFluidAmount(), FLUID_TRANSFER[level]));
		for (int i = outputTrackerFluid + 1; i <= outputTrackerFluid + 6; i++) {
			side = i % 6;
			if (isSecondaryOutput(sideConfig.sideTypes[sideCache[side]])) {
				int toDrain = FluidHelper.insertFluidIntoAdjacentFluidHandler(this, EnumFacing.VALUES[side],
						outputBuffer, true);
				if (toDrain > 0) {
	...
	private void transferHandler() {
		...
		if (hasFluidHandler) {
			if (inventory[2].isEmpty()) {
				...
				if (ItemHelper.itemsIdentical(inventory[1], inventory[2]) && inventory[1].getMaxStackSize() > 1
	...
	public Object getGuiClient(InventoryPlayer inventory) {
		...
		return new GuiCrystaliser(inventory, this);
	}

	@Override
	public Object getGuiServer(InventoryPlayer inventory) {

		return new ContainerCrystaliser(inventory, this);
	}
	...
	public void readFromNBT(NBTTagCompound nbt) {
		...
		if (!inventory[1].isEmpty()
				&& inventory[1].hasCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY, null)) {
			hasFluidHandler = true;
		}
	...
	public TextureAtlasSprite getTexture(int side, int pass) {
		...
		if (pass == 0) {
			...
			return side != facing ? TETextures.MACHINE_SIDE
					: isActive ? RenderHelper.getFluidTexture(renderFluid) : TETextures.MACHINE_FACE[TYPE];
		} else if (side < 6) {
			return side != facing ? TETextures.CONFIG[sideConfig.sideTypes[sideCache[side]]]
					: isActive ? TETextures.MACHINE_ACTIVE[TYPE] : TETextures.MACHINE_FACE[TYPE];
		}
	...
	public int getColorMask(BlockRenderLayer layer, EnumFacing side) {
		...
		return layer == BlockRenderLayer.SOLID && side.ordinal() == facing && isActive
				? renderFluid.getFluid().getColor(renderFluid) << 8 | 0xFF
				: super.getColorMask(layer, side);
	}
